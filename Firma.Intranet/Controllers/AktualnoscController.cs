...
namespace Firma.Intranet.Controllers
{
    public class AktualnoscController : Controller
    {
        ...
        // to jest obiekt odpowiedzialny z sapołaczenie z bazą danych
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var aktualnosc = await _context.Aktualnosc
                .FirstOrDefaultAsync(m => m.IdAktulanosci == id);
            if (aktualnosc == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> Create([Bind("IdAktulanosci,LinkTytul,Tytul,Tresc,Pozycja")] Aktualnosc aktualnosc)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("IdAktulanosci,LinkTytul,Tytul,Tresc,Pozycja")] Aktualnosc aktualnosc)
        {
            if (id != aktualnosc.IdAktulanosci)
            {
                return NotFound();
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    if (!AktualnoscExists(aktualnosc.IdAktulanosci))
                    {
                        return NotFound();
                    }
                    else
        ...
        {
            return _context.Aktualnosc.Any(e => e.IdAktulanosci == id);
        }
    }
}
